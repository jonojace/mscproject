...
python plot_binary_file.py .../binary_label_450/arctic_a0001.lab 450 441

where 450 is the total number of features in the binary file
and 440 is the individual features that you wish to plot
'''
...
#    data2 = get_speech(param_file2, dim, remove_htk_header=False, double_precision=True)
    ...
    # print 'plotting feature:', which_dim
    # plot(data[:, which_dim])

    # print 'plotting features for debug'
    # plot(data[:, 430:])
...
def get_speech(infile, dim, remove_htk_header=False, double_precision=False):
    ...
    #print len(data)
    #print len(data) / float(dim)
    print 'len(data)', len(data), 'float(dim)', float(dim)
