...
# ********************************************************************************
# This differs from synthesise_tp_define_sent_embedding.sh
# as it you can't use any arbitrary CV, can only use utt_id to specify which CV to use
#
# we use this script to test the TP model and see how well it can generate with
# different features, look inside append_TP_feats_TestTime.py for more info about
...
mkdir -p experiments/${VOICE_NAME}/test_synthesis/txt/ #make the directory if it doesn't exist
...
python ${SCRIPT_DIR}/append_TP_feats_TestTime.py ${PROMPT_LAB_FOLDER}

echo "synthesizing durations..."
./scripts/submit.sh ${MerlinDir}/src/run_merlin_tp.py $test_dur_config_file

echo "synthesizing speech..."
./scripts/submit.sh ${MerlinDir}/src/run_merlin_tp.py $test_synth_config_file
